...
func (dplr *deployer) postWebHook(w http.ResponseWriter, r *http.Request) {
	...
	if hookData.Repository.Namespace != `angrypapa` {
...
func deploy() {
	...
	params := &elasticbeanstalk.UpdateEnvironmentInput{
		ApplicationName: aws.String("deployer202204101625"),
		EnvironmentId:   aws.String("e-c9kjgygryn"),
...
"source": "https://github.com/angrypapa/deployer",
"version": "%s",
"commit": "%s",
"build": "https://circleci.com/gh/angrypapa/deployer/"
}`, version, commit)))
}
